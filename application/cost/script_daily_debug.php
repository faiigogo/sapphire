...
$non_session = "ON";
$Report_Now = "ON";  // OFF = ส่งเมล์และsmsได้  	ON = เรียกดูรายงานเฉยๆ
$Fix_Hour = "ON";		  // OFF = ไม่สนใจเวลา  		ON = สนใจเวลา
$Debug_Mode = "ON";  // OFF = ปิดตอนแก้โปรแกรม	ON = เปิดตอนแก้โปรแกรม

$debug = "ON";			// OFF = "ปิดโชว์ QUERY"		ON = "เปิดโชว์ QUERY"
	...
	$Result = mysql_query($strSQL);
		...
		while($Row_trip = mysql_fetch_assoc($Result)){
			...
			if(@mysql_num_rows($Result_schedule)>0){
				...
				echo "$maxdate,$nowdate && $maxdate<$nowdate <br>";
				### คำนวณทริปช้า

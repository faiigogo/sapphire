...
if ($_SERVER[REQUEST_METHOD] == "POST"){
		 if ($action =="close")
		 {
				...
				if( $list_check == "ON" ){
					...
					$strSQL = " SELECT * from list where runno in ($no_list) group by tripid";
					$result = @mysql_query($strSQL);
					while($result_runno = @mysql_fetch_assoc($result)){
...
function checkAll(obj, id){
	...
	for(i=0;i<tagMeta.length;i++){	
		if(tagMeta[i].type=='checkbox'){
			if(obj.checked == true){
				document.getElementById(tagMeta[i].id).checked = true;
			}else{
				document.getElementById(tagMeta[i].id).checked = false;
			}
		}
	}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	}elseif (($action == "search") and ($show == "all"))  // คิวรี่ตามเอกสารที่สรุปค่าใช้จ่ายเสร็จสิ้น
	{
		$str = "select * from list where (date_list between '$startday' and '$endday') ";
		$word = "รายงานแสดงรายการตามเอกสารที่สรุปค่าใช้จ่ายเสร็จสิ้น ตั้งแต่วันที่".daythai($startday)." ถึง วันที่".daythai($endday)   ;		
	}else
	{
...
?>
							...
							<?
							  ...
							  ?>
                          <br>
                          <a href="cleartripvalue.php?tripid=<?=$tripid?>">ตรวจเช็ครายการโอนเงิน</a>
                          <br />
							 </td>
                              <td>&nbsp;</td>
                              <td>&nbsp;</td>
                          ...
                          <table width="98%" border="0" cellspacing="2" cellpadding="2" align="center" bgcolor="black" id="tbl_data">
                            <tr bgcolor="#A3B2CC" align="center">
                              <td width="3%" rowspan="3">เลือกรายการ
                              <br/>
                              <input type="checkbox" id="checkall" name="checkall" value="all" onClick="checkAll(this,'tbl_data');" />
                              </td>
...
?>
                            <tr bgcolor="<?=$bg?>" align="center">
                              <td width="3%"><label>
                                <input type="checkbox" id="checkbox<?=$i?>" name="checkbox[]" value="<?=$rs[runno]?>"  <? if ($c =="0") echo "disabled";?>>
                              </label></td>
                              <td width="3%"><?=$no?></td>

...
/*if (!$db){
	echo "cannot connect to database servers";
	exit;
}
*/


$sqlcheck = "select * from sysobjects;";
echo "$sqlcheck<BR><BR><PRE>";
$rsACC = $db->Execute($sqlcheck);
 while (!$rsACC->EOF)
 { 

	 print_r($rsACC->Fields);
   // echo $rsACC->Fields['RecID']->Value . " " . $rsACC->Fields['System']->Value . "" .  $rsACC->Fields['RecHDID']->Value."<BR>";
    $rsACC->MoveNext();
 }	
$rsACC->Close();
...
<?
		//// write ลง Table ///////////////
/*
$sqlDT = "SELECT  *  FROM ShowGL_DT  Where  RecHDID  = '$RecHD'  ";
$rsDT  = $db->Execute($sqlDT); 
*/
...
{
$sqlinsert = "UPDATE  ShowGL_DT SET  RecHDID = $RecHDID ,AccCoaID = $AccCoaID,ProjID  =  $ProjID,DebitAmnt  = $DebitAmnt,CreditAmnt  = $CreditAmnt,BranchIDFrom  =$BranchIDFrom ,SideID = $SideID,DeptID = $DeptID,Active = $Active,UserLogin = '$UserLogin',UserAdjust = '$UserAdjust' ,AdjNew  = $AdjNew,AdjFlag  = $AdjFlag ,TimeAdj = '$TimeAdj'  where  RecHDID = '$RecHDID'  ";
}

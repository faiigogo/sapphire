...
$arr_type = array("PRO"=>"โครงการ","PS"=>"Presale","OF"=>"ออฟฟิศ","MAR"=>"ตลาด","RD"=>"วิจัยพัฒนา","MA"=>"อยู่ในประกัน");
...
if ($_POST[action]=="edit2"){
				...
				#Begin Update โครงการบริษัท GNIS
				...
				mysql_db_query($db_name_gnis, $sql);//$sql update ให้เหมือนบริษัท Sapphire
				#End Update โครงการบริษัท GNIS
				...
				// -------------- กรณีที่เปลี่ยนเป็น Project ----------------------------- // 
						if(isset($presale2project)){
							#Begin Database: Sapphire เพิ่มรหัสอ้างอิงในช่อง detail ในตาราง  list และเพิ่มข้อมูลในช่อง presale_code ในตาราง type_project 
							...
							#End Database: Sapphire เพิ่มรหัสอ้างอิงในช่อง detail ในตาราง  list และเพิ่มข้อมูลในช่อง presale_code ในตาราง type_project 
							
							#Begin Database: GNIS เพิ่มรหัสอ้างอิงในช่อง detail ในตาราง  list และเพิ่มข้อมูลในช่อง presale_code ในตาราง type_project 
							$str = " select *  from list t1 left join type_project t2 on t1.id_type_project = t2.id_type_project where  t1. id_type_project='$id_type_project'   ";
							...
							#End Database: GNIS เพิ่มรหัสอ้างอิงในช่อง detail ในตาราง  list และเพิ่มข้อมูลในช่อง presale_code ในตาราง type_project						
						} // end if 
				...
				$sql = "INSERT INTO  type_project (id_type_project,code_project,name_project,no_project,value,start_project,end_project,projecttype,province,province_code)	VALUES ('$id_type_project','".trim($code_project)."','$name_project','$no_project','$value','$start_project','$end_project','$projecttype','$province','$province_code')";
					...
					if($result){
						$query = mysql_db_query($db_name, "select id_type_project from  type_project   ;");//Connect ฐานข้อมูลกลับ $db_name
						//header("Location: ?id=$id&action=edit&refreshpage=1");
						echo "<script>window.location='?id=$id&action=edit&refreshpage=1';</script>";
						exit;
					}else{	
...
?>
        <td valign="top" >
        <p><? include("header_cost.php"); // หัวโปรแกรม ?></p>
        <table width="80%" border="0" align="center">
          <tr>
            <td>
            <form action="" method="get">
            <fieldset>
            <legend>ค้นหาข้อมูลโครงการ</legend>
            <table width="100%" border="0">
              <tr>
                <td width="150" align="right">ชื่อโครงการ:</td>
                <td><input type="text" name="name_project" value="<?php echo $_GET['name_project'];?>" size="50"/></td>
              </tr>
              <tr>
                <td align="right">ปีเริ่มต้นสัญญา:</td>
                <td>
                <select name="yy_s">
              ...
              <tr>
                <td align="right">ประเภทโครงการ:</td>
                <td>
                <select name="type_p">
				<option value="">ไม่ระบุ</option>
                <?php 
              ...
              <tr>
                <td>&nbsp;</td>
                <td>
                <input type="submit" name="b_search" value="บันทึก"/>&nbsp;<input type="button" name="b_cancel" value="ยกเลิก" onClick="javascript:window.location='?';"/>
                </td>
              </tr>
            </table>
            </fieldset>
            ...
            <form name="form1"  method = POST  action = "<?=$PHP_SELF?>">
              ...
              <table width="100%" border="0" cellspacing="1" cellpadding="2" align="center">
                ...
                <tr>
                  ...
                  <input type="button" name="Button" onClick="conp2o()" value="แก้ไขโครงการนี้ให้เป็นค่าใช้จ่าย office">
                  <script language="javascript">
				    function conp2o(){
						if(confirm(" แก้ไขโครงการนี้ให้เป็นค่าใช้จ่าย office ? ")==true){

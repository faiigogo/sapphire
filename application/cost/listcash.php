...
?>
        ...
        <tr>
          <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				?>
                            ...
                            <tr>
                              <td width="30%" align="right"><strong>งบประมาณที่ใช้จริง - <a href="listcash.php?mode=total&tripid=<?=$tripid?>">เงินสด</a> </strong></td>
                              <td width="25%" align="right" bgcolor="#FFFFFF"><span class="style2">
                                <?
				If ($pri=='100')
				{
				$sqls = "select sum(cash_total) as cash_total,sum(credit_total) as credit_total  from list  where tripid = '$tripid'   and cash_check ='1' ";
				}else
				{
				$sqls = "select sum(cash_total) as cash_total,sum(credit_total) as credit_total  from list  where tripid = '$tripid'  and userid= '$_SESSION[userid]' and cash_check ='1' ";
				}
				$res = mysql_query($sqls);
				...
				?>
                            ...
                            <tr>
                              <td width="30%" align="right"><strong>- <a href="listcredit.php?mode=total&tripid=<?=$tripid?>">เครดิต</a> </strong></td>
                          ...
                          <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="50%" height="25"><strong>รายการค่าใช้จ่ายประเภทเงินสด</strong></td>
                            </tr>
                          </table>
                          <table width="98%" border="0" align="center" cellpadding="1" cellspacing="2" bgcolor="#000000">
                            <tr>
                              ...
                              <td colspan="3" align="center" bgcolor="#A3B2CC"><strong>จำนวนเงินCASH</strong></td>
	...
	If  ($pri == '100')
	{
		...
		If ($sortfield == "")
		{
			$str = "select *  from list t1 left join type_project t2 on t1.id_type_project = t2.id_type_project where (t1.tripid = '$tripid') and (t1. cash_check='1')  order by date_list  ";
		}
		else
		{
		$str = " select *  from list t1 left join type_project t2 on t1.id_type_project = t2.id_type_project where (t1.tripid = '$tripid') and (t1. cash_check='1')  order by $sortfield $sort   ";
		}
	}else
	{
		If ($sortfield == "")
		{
			$str = "select *  from list t1 left join type_project t2 on t1.id_type_project = t2.id_type_project where (t1.tripid = '$tripid') and (t1. cash_check='1') and (t1.userid = '$userid') order by date_list  ";
		}
		else
		{
		$str = " select *  from list t1 left join type_project t2 on t1.id_type_project = t2.id_type_project where (t1.tripid = '$tripid') and (t1. cash_check='1') and (t1.userid = '$userid') order by $sortfield $sort   ";
		}
...
?>
                              ...
                              <td align="right" bgcolor="<?=$bg?>"><?=number_format($rs[cash],2)?></td>
                              <td align="right" bgcolor="<?=$bg?>"><?=number_format($rs[cash_vat],2)?></td>
